    ...
    async.series({
        ...
        prepareData0: function (cb) {
            readyData = {
                ...
                finish_date: calendar.closing_date,
                payment_amount: calendar.payments_count,
                payment: []
            };
            cb(null);
        },
        ...
        prepareTemplate: function (cb) {
            fs.readFile('./templates/merchant_send_calendar.html', function (err, data) {
                ...
                var m_obj = {
                    ...
                    agreement_sign_date: merchant_financing.agreement_sign_date || '______'
                };
                tpl = mustache.to_html(tpl, m_obj);
                cb(null);
            });
        ...
        sendEmailToBank: function (cb) {
            // Отравить на емайл
            sendMail({email: 'ivantgco@gmail.com', html: tpl, attachments: attachments}, function (err, info) {
                if (err) return cb(new UserError('Не удалось отправить email', {err: err, info: info}));
                cb(null);
            });
        },
